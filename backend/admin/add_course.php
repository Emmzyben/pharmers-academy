...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($courseName) && !empty($description) && is_array($modules) && is_array($descriptions) && !empty($price)) {
        // Insert the course into the database first
        $queryCourse = "INSERT INTO courses (course_name, description, price) VALUES (?, ?, ?)";
        $stmtCourse = $conn->prepare($queryCourse);

        if ($stmtCourse) {
            $stmtCourse->bind_param("sss", $courseName, $description, $price);
