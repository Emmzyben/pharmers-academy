...
if ($id) {
    ...
    $stmt = $conn->prepare("SELECT id,firstName, lastName, contact, location, degree, course, profile_picture FROM instructors WHERE id=?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->bind_result($admin_id, $admin_firstName, $admin_lastName,  $admin_contact, $admin_location, $admin_degree, $admin_course, $admin_profilePicture);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->num_rows > 0) {
        ...
        $stmt->close();
        header("Location: instructor_detail.php?id=$id");
        exit();
    }
    $stmt->close();
    ...
    if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] == 0) {
        ...
        if (!in_array($fileExtension, $allowedExtensions)) {
            ...
            header("Location: instructor_detail.php");
            exit();
        }
    ...
    $stmt = $conn->prepare("UPDATE instructors SET  firstname=?, lastname=?, contact=?, location=?, degree=?, course=? WHERE id=?");
    $stmt->bind_param("ssssssi", $firstName, $lastName,  $contact, $location, $degree, $course, $id);

    if (!$stmt->execute()) {
        setSessionMessage("Error updating instructor record: " . $stmt->error, "error");
        $stmt->close();
        header("Location: instructor_detail.php?id=$id");
        exit();
    }
    ...
    if ($stmt->execute()) {
        setSessionMessage("Instructor record updated successfully.", "success");
        $stmt->close();
            ...
            <div class="col-md-12 grid-margin">
              <div class="row">
                <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                  <h3 class="font-weight-bold">Instructor Details</h3>
                  <h6 class="font-weight-normal mb-0">View/edit Instructor details</h6>
                </div>
                    ...
                    <div class="row">
                      ...
                      <div class="col-md-6">
                        <div class="form-group row">
                          ...
                          <div class="col-sm-9">
                          <input type="text" name='password' class="form-control" value="<?php echo htmlspecialchars($admin2['password']); ?>">
                          </div>
                        </div>
                      </div>
                    </div>
                      ...
                      <div class="col-md-6">
                        <div class="form-group row">
                          <div class="col-sm-9">
                            ...
                            <button type="submit" class="btn btn-danger mr-2">Delete Instructor</button>
                          </div>
                        </div>
                        <script>
    ...
    if (file) {
      ...
      reader.readAsDataURL(file);
            ...
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <p class="card-title">Students Handled</p>
                  <div class="row">
                    <div class="col-12">
